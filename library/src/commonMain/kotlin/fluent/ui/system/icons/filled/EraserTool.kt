package fluent.ui.system.icons.filled

import org.jetbrains.compose.ui.tooling.preview.Preview
import androidx.compose.foundation.Image
import androidx.compose.foundation.layout.Box
import androidx.compose.foundation.layout.padding
import androidx.compose.runtime.Composable
import androidx.compose.ui.Modifier
import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import fluent.ui.system.icons.FluentUi

public val FluentUi.Filled.EraserTool: ImageVector by lazy(LazyThreadSafetyMode.NONE) {
    ImageVector.Builder(
        name = "Filled.EraserTool",
        defaultWidth = 24.dp,
        defaultHeight = 24.dp,
        viewportWidth = 24f,
        viewportHeight = 24f
    ).apply {
        path(fill = SolidColor(Color(0xFF212121))) {
            moveTo(4.5f, 2.75f)
            curveTo(4.5f, 2.336f, 4.164f, 2f, 3.75f, 2f)
            curveTo(3.336f, 2f, 3f, 2.336f, 3f, 2.75f)
            verticalLineTo(17.25f)
            curveTo(3f, 19.873f, 5.127f, 22f, 7.75f, 22f)
            horizontalLineTo(16.25f)
            curveTo(18.873f, 22f, 21f, 19.873f, 21f, 17.25f)
            verticalLineTo(2.75f)
            curveTo(21f, 2.336f, 20.664f, 2f, 20.25f, 2f)
            curveTo(19.836f, 2f, 19.5f, 2.336f, 19.5f, 2.75f)
            verticalLineTo(7f)
            horizontalLineTo(4.5f)
            verticalLineTo(2.75f)
            close()
            moveTo(4.5f, 8.5f)
            horizontalLineTo(19.5f)
            verticalLineTo(12f)
            horizontalLineTo(4.5f)
            verticalLineTo(8.5f)
            close()
        }
    }.build()
}

@Preview
@Composable
private fun EraserToolPreview() {
    Box(modifier = Modifier.padding(12.dp)) {
        Image(imageVector = FluentUi.Filled.EraserTool, contentDescription = null)
    }
}
