package fluent.ui.system.icons.regular

import androidx.compose.foundation.Image
import androidx.compose.foundation.layout.Box
import androidx.compose.foundation.layout.padding
import androidx.compose.runtime.Composable
import androidx.compose.ui.Modifier
import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import org.jetbrains.compose.ui.tooling.preview.Preview
import androidx.compose.ui.unit.dp
import fluent.ui.system.icons.FluentIcons

public val FluentIcons.Regular.CheckmarkCircleWarning: ImageVector
    get() {
        if (_CheckmarkCircleWarning != null) {
            return _CheckmarkCircleWarning!!
        }
        _CheckmarkCircleWarning = ImageVector.Builder(
            name = "Regular.CheckmarkCircleWarning",
            defaultWidth = 24.dp,
            defaultHeight = 24.dp,
            viewportWidth = 24f,
            viewportHeight = 24f,
        ).apply {
            path(fill = SolidColor(Color(0xFF212121))) {
                moveTo(12f, 2f)
                curveTo(17.523f, 2f, 22f, 6.477f, 22f, 12f)
                curveTo(22f, 13.042f, 21.841f, 14.047f, 21.545f, 14.991f)
                lineTo(20.741f, 13.383f)
                curveTo(20.658f, 13.216f, 20.56f, 13.063f, 20.45f, 12.926f)
                curveTo(20.483f, 12.622f, 20.5f, 12.313f, 20.5f, 12f)
                curveTo(20.5f, 7.306f, 16.694f, 3.5f, 12f, 3.5f)
                curveTo(7.306f, 3.5f, 3.5f, 7.306f, 3.5f, 12f)
                curveTo(3.5f, 16.694f, 7.306f, 20.5f, 12f, 20.5f)
                curveTo(12.245f, 20.5f, 12.487f, 20.49f, 12.726f, 20.469f)
                lineTo(12.271f, 21.381f)
                curveTo(12.169f, 21.584f, 12.098f, 21.792f, 12.055f, 22f)
                lineTo(12f, 22f)
                curveTo(6.477f, 22f, 2f, 17.523f, 2f, 12f)
                curveTo(2f, 6.477f, 6.477f, 2f, 12f, 2f)
                close()
                moveTo(10.75f, 13.439f)
                lineTo(15.22f, 8.97f)
                curveTo(15.513f, 8.677f, 15.987f, 8.677f, 16.28f, 8.97f)
                curveTo(16.547f, 9.236f, 16.571f, 9.653f, 16.353f, 9.946f)
                lineTo(16.28f, 10.03f)
                lineTo(11.28f, 15.03f)
                curveTo(11.014f, 15.297f, 10.597f, 15.321f, 10.304f, 15.103f)
                lineTo(10.22f, 15.03f)
                lineTo(7.72f, 12.53f)
                curveTo(7.427f, 12.237f, 7.427f, 11.763f, 7.72f, 11.47f)
                curveTo(7.986f, 11.203f, 8.403f, 11.179f, 8.696f, 11.397f)
                lineTo(8.78f, 11.47f)
                lineTo(10.75f, 13.439f)
                close()
                moveTo(17.161f, 13.83f)
                curveTo(17.714f, 12.723f, 19.294f, 12.723f, 19.847f, 13.83f)
                lineTo(23.844f, 21.828f)
                curveTo(24.342f, 22.826f, 23.617f, 24f, 22.501f, 24f)
                horizontalLineTo(14.507f)
                curveTo(13.391f, 24f, 12.665f, 22.826f, 13.164f, 21.828f)
                lineTo(17.161f, 13.83f)
                close()
                moveTo(19.004f, 16.496f)
                curveTo(19.004f, 16.219f, 18.78f, 15.995f, 18.504f, 15.995f)
                curveTo(18.228f, 15.995f, 18.003f, 16.219f, 18.003f, 16.496f)
                verticalLineTo(19.497f)
                curveTo(18.003f, 19.774f, 18.228f, 19.998f, 18.504f, 19.998f)
                curveTo(18.78f, 19.998f, 19.004f, 19.774f, 19.004f, 19.497f)
                verticalLineTo(16.496f)
                close()
                moveTo(18.504f, 21.999f)
                curveTo(18.78f, 21.999f, 19.004f, 21.775f, 19.004f, 21.499f)
                curveTo(19.004f, 21.222f, 18.78f, 20.998f, 18.504f, 20.998f)
                curveTo(18.228f, 20.998f, 18.003f, 21.222f, 18.003f, 21.499f)
                curveTo(18.003f, 21.775f, 18.228f, 21.999f, 18.504f, 21.999f)
                close()
            }
        }.build()

        return _CheckmarkCircleWarning!!
    }

@Suppress("ObjectPropertyName")
private var _CheckmarkCircleWarning: ImageVector? = null

@Preview
@Composable
private fun CheckmarkCircleWarningPreview() {
    Box(modifier = Modifier.padding(12.dp)) {
        Image(imageVector = FluentIcons.Regular.CheckmarkCircleWarning, contentDescription = null)
    }
}
