package fluent.ui.system.icons.regular

import androidx.compose.foundation.Image
import androidx.compose.foundation.layout.Box
import androidx.compose.foundation.layout.padding
import androidx.compose.runtime.Composable
import androidx.compose.ui.Modifier
import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import org.jetbrains.compose.ui.tooling.preview.Preview
import androidx.compose.ui.unit.dp
import fluent.ui.system.icons.FluentIcons

public val FluentIcons.Regular.PersonAlertOff: ImageVector
    get() {
        if (_PersonAlertOff != null) {
            return _PersonAlertOff!!
        }
        _PersonAlertOff = ImageVector.Builder(
            name = "Regular.PersonAlertOff",
            defaultWidth = 24.dp,
            defaultHeight = 24.dp,
            viewportWidth = 24f,
            viewportHeight = 24f,
        ).apply {
            path(fill = SolidColor(Color(0xFF212121))) {
                moveTo(11.939f, 14.061f)
                curveTo(11.92f, 14.041f, 11.9f, 14.021f, 11.882f, 14f)
                horizontalLineTo(4.253f)
                curveTo(3.011f, 14f, 2.004f, 15.007f, 2.004f, 16.249f)
                verticalLineTo(16.826f)
                curveTo(2.004f, 17.719f, 2.322f, 18.583f, 2.902f, 19.262f)
                curveTo(4.468f, 21.096f, 6.854f, 22.001f, 10f, 22.001f)
                curveTo(10.67f, 22.001f, 11.306f, 21.96f, 11.906f, 21.878f)
                curveTo(11.891f, 21.871f, 11.876f, 21.865f, 11.86f, 21.858f)
                curveTo(11.307f, 21.597f, 10.986f, 21.041f, 10.998f, 20.466f)
                curveTo(10.678f, 20.489f, 10.345f, 20.501f, 10f, 20.501f)
                curveTo(7.262f, 20.501f, 5.296f, 19.755f, 4.043f, 18.288f)
                curveTo(3.695f, 17.88f, 3.504f, 17.362f, 3.504f, 16.826f)
                verticalLineTo(16.249f)
                curveTo(3.504f, 15.835f, 3.839f, 15.5f, 4.253f, 15.5f)
                horizontalLineTo(12.526f)
                curveTo(12.55f, 15.252f, 12.593f, 15.009f, 12.652f, 14.774f)
                lineTo(11.939f, 14.061f)
                close()
                moveTo(10f, 2.005f)
                curveTo(12.762f, 2.005f, 15f, 4.243f, 15f, 7.005f)
                curveTo(15f, 9.766f, 12.762f, 12.005f, 10f, 12.005f)
                curveTo(7.239f, 12.005f, 5f, 9.766f, 5f, 7.005f)
                curveTo(5f, 4.243f, 7.239f, 2.005f, 10f, 2.005f)
                close()
                moveTo(10f, 3.505f)
                curveTo(8.067f, 3.505f, 6.5f, 5.072f, 6.5f, 7.005f)
                curveTo(6.5f, 8.938f, 8.067f, 10.505f, 10f, 10.505f)
                curveTo(11.933f, 10.505f, 13.5f, 8.938f, 13.5f, 7.005f)
                curveTo(13.5f, 5.072f, 11.933f, 3.505f, 10f, 3.505f)
                close()
                moveTo(13.502f, 16f)
                curveTo(13.502f, 15.47f, 13.606f, 14.963f, 13.793f, 14.5f)
                lineTo(12.646f, 13.354f)
                curveTo(12.451f, 13.158f, 12.451f, 12.842f, 12.646f, 12.646f)
                curveTo(12.842f, 12.451f, 13.158f, 12.451f, 13.354f, 12.646f)
                lineTo(22.354f, 21.646f)
                curveTo(22.549f, 21.842f, 22.549f, 22.158f, 22.354f, 22.354f)
                curveTo(22.158f, 22.549f, 21.842f, 22.549f, 21.646f, 22.354f)
                lineTo(20.293f, 21f)
                horizontalLineTo(12.993f)
                curveTo(12.59f, 21f, 12.389f, 21f, 12.288f, 20.952f)
                curveTo(12.039f, 20.836f, 11.931f, 20.541f, 12.046f, 20.291f)
                curveTo(12.092f, 20.19f, 12.246f, 20.06f, 12.554f, 19.8f)
                lineTo(13.502f, 19f)
                lineTo(13.502f, 16f)
                close()
                moveTo(21.503f, 18f)
                verticalLineTo(16f)
                curveTo(21.503f, 13.791f, 19.712f, 12f, 17.503f, 12f)
                curveTo(16.556f, 12f, 15.686f, 12.329f, 15f, 12.879f)
                lineTo(21.5f, 19.379f)
                verticalLineTo(18f)
                horizontalLineTo(21.503f)
                close()
                moveTo(17.503f, 23f)
                curveTo(18.195f, 23f, 18.784f, 22.583f, 19.003f, 22f)
                horizontalLineTo(16.003f)
                curveTo(16.221f, 22.583f, 16.81f, 23f, 17.503f, 23f)
                close()
            }
        }.build()

        return _PersonAlertOff!!
    }

@Suppress("ObjectPropertyName")
private var _PersonAlertOff: ImageVector? = null

@Preview
@Composable
private fun PersonAlertOffPreview() {
    Box(modifier = Modifier.padding(12.dp)) {
        Image(imageVector = FluentIcons.Regular.PersonAlertOff, contentDescription = null)
    }
}
