package fluent.ui.system.icons.regular

import androidx.compose.foundation.Image
import androidx.compose.foundation.layout.Box
import androidx.compose.foundation.layout.padding
import androidx.compose.runtime.Composable
import androidx.compose.ui.Modifier
import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import org.jetbrains.compose.ui.tooling.preview.Preview
import androidx.compose.ui.unit.dp
import fluent.ui.system.icons.FluentIcons

public val FluentIcons.Regular.DataUsageSettings: ImageVector
    get() {
        if (_DataUsageSettings != null) {
            return _DataUsageSettings!!
        }
        _DataUsageSettings = ImageVector.Builder(
            name = "Regular.DataUsageSettings",
            defaultWidth = 24.dp,
            defaultHeight = 24.dp,
            viewportWidth = 24f,
            viewportHeight = 24f,
        ).apply {
            path(fill = SolidColor(Color(0xFF212121))) {
                moveTo(17.25f, 2.996f)
                curveTo(18.769f, 2.996f, 20f, 4.227f, 20f, 5.746f)
                verticalLineTo(11.498f)
                curveTo(19.526f, 11.3f, 19.023f, 11.157f, 18.5f, 11.076f)
                verticalLineTo(5.746f)
                curveTo(18.5f, 5.056f, 17.94f, 4.496f, 17.25f, 4.496f)
                horizontalLineTo(4.75f)
                curveTo(4.06f, 4.496f, 3.5f, 5.056f, 3.5f, 5.746f)
                verticalLineTo(18.246f)
                curveTo(3.5f, 18.937f, 4.06f, 19.496f, 4.75f, 19.496f)
                horizontalLineTo(11.312f)
                curveTo(11.484f, 20.029f, 11.723f, 20.533f, 12.019f, 20.996f)
                horizontalLineTo(4.75f)
                curveTo(3.231f, 20.996f, 2f, 19.765f, 2f, 18.246f)
                verticalLineTo(5.746f)
                curveTo(2f, 4.227f, 3.231f, 2.996f, 4.75f, 2.996f)
                horizontalLineTo(17.25f)
                close()
                moveTo(16f, 7.729f)
                verticalLineTo(11.174f)
                curveTo(15.472f, 11.299f, 14.969f, 11.488f, 14.5f, 11.732f)
                verticalLineTo(7.729f)
                curveTo(14.5f, 7.326f, 14.836f, 6.999f, 15.25f, 6.999f)
                curveTo(15.63f, 6.999f, 15.943f, 7.274f, 15.993f, 7.63f)
                lineTo(16f, 7.729f)
                close()
                moveTo(11.715f, 12.715f)
                lineTo(11.732f, 14.5f)
                curveTo(11.338f, 15.257f, 11.088f, 16.101f, 11.019f, 16.996f)
                curveTo(10.651f, 16.997f, 10.345f, 16.728f, 10.293f, 16.375f)
                lineTo(10.285f, 16.277f)
                lineTo(10.25f, 12.73f)
                curveTo(10.246f, 12.328f, 10.571f, 12f, 10.975f, 11.996f)
                curveTo(11.346f, 11.993f, 11.655f, 12.263f, 11.707f, 12.617f)
                lineTo(11.715f, 12.715f)
                close()
                moveTo(6.75f, 8.999f)
                curveTo(7.13f, 8.999f, 7.443f, 9.28f, 7.493f, 9.645f)
                lineTo(7.5f, 9.746f)
                verticalLineTo(16.253f)
                curveTo(7.5f, 16.665f, 7.164f, 16.999f, 6.75f, 16.999f)
                curveTo(6.37f, 16.999f, 6.057f, 16.719f, 6.007f, 16.354f)
                lineTo(6f, 16.253f)
                verticalLineTo(9.746f)
                curveTo(6f, 9.334f, 6.336f, 8.999f, 6.75f, 8.999f)
                close()
                moveTo(14.278f, 13.976f)
                curveTo(14.593f, 15.066f, 13.939f, 16.199f, 12.837f, 16.472f)
                lineTo(12.253f, 16.616f)
                curveTo(12.208f, 16.904f, 12.185f, 17.199f, 12.185f, 17.5f)
                curveTo(12.185f, 17.815f, 12.21f, 18.123f, 12.259f, 18.424f)
                lineTo(12.799f, 18.553f)
                curveTo(13.911f, 18.821f, 14.573f, 19.966f, 14.249f, 21.064f)
                lineTo(14.062f, 21.695f)
                curveTo(14.501f, 22.081f, 15.002f, 22.394f, 15.546f, 22.617f)
                lineTo(16.04f, 22.098f)
                curveTo(16.828f, 21.269f, 18.151f, 21.269f, 18.939f, 22.098f)
                lineTo(19.438f, 22.623f)
                curveTo(19.981f, 22.403f, 20.481f, 22.093f, 20.92f, 21.71f)
                lineTo(20.722f, 21.024f)
                curveTo(20.408f, 19.934f, 21.062f, 18.801f, 22.163f, 18.528f)
                lineTo(22.747f, 18.384f)
                curveTo(22.792f, 18.096f, 22.816f, 17.801f, 22.816f, 17.5f)
                curveTo(22.816f, 17.185f, 22.79f, 16.877f, 22.741f, 16.576f)
                lineTo(22.202f, 16.447f)
                curveTo(21.089f, 16.179f, 20.428f, 15.034f, 20.752f, 13.936f)
                lineTo(20.938f, 13.305f)
                curveTo(20.499f, 12.92f, 19.999f, 12.606f, 19.454f, 12.384f)
                lineTo(18.961f, 12.902f)
                curveTo(18.172f, 13.731f, 16.85f, 13.731f, 16.062f, 12.902f)
                lineTo(15.563f, 12.377f)
                curveTo(15.019f, 12.597f, 14.519f, 12.907f, 14.08f, 13.289f)
                lineTo(14.278f, 13.976f)
                close()
                moveTo(17.5f, 19f)
                curveTo(16.699f, 19f, 16.05f, 18.329f, 16.05f, 17.5f)
                curveTo(16.05f, 16.672f, 16.699f, 16f, 17.5f, 16f)
                curveTo(18.301f, 16f, 18.95f, 16.672f, 18.95f, 17.5f)
                curveTo(18.95f, 18.329f, 18.301f, 19f, 17.5f, 19f)
                close()
            }
        }.build()

        return _DataUsageSettings!!
    }

@Suppress("ObjectPropertyName")
private var _DataUsageSettings: ImageVector? = null

@Preview
@Composable
private fun DataUsageSettingsPreview() {
    Box(modifier = Modifier.padding(12.dp)) {
        Image(imageVector = FluentIcons.Regular.DataUsageSettings, contentDescription = null)
    }
}
