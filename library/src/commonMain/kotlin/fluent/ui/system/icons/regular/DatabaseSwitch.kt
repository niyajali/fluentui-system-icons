package fluent.ui.system.icons.regular

import androidx.compose.foundation.Image
import androidx.compose.foundation.layout.Box
import androidx.compose.foundation.layout.padding
import androidx.compose.runtime.Composable
import androidx.compose.ui.Modifier
import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import org.jetbrains.compose.ui.tooling.preview.Preview
import androidx.compose.ui.unit.dp
import fluent.ui.system.icons.FluentIcons

public val FluentIcons.Regular.DatabaseSwitch: ImageVector
    get() {
        if (_DatabaseSwitch != null) {
            return _DatabaseSwitch!!
        }
        _DatabaseSwitch = ImageVector.Builder(
            name = "Regular.DatabaseSwitch",
            defaultWidth = 24.dp,
            defaultHeight = 24.dp,
            viewportWidth = 24f,
            viewportHeight = 24f,
        ).apply {
            path(fill = SolidColor(Color(0xFF212121))) {
                moveTo(4f, 6f)
                curveTo(4f, 5.309f, 4.315f, 4.707f, 4.774f, 4.22f)
                curveTo(5.229f, 3.738f, 5.853f, 3.337f, 6.567f, 3.018f)
                curveTo(7.996f, 2.377f, 9.917f, 2f, 12f, 2f)
                curveTo(14.083f, 2f, 16.004f, 2.377f, 17.433f, 3.018f)
                curveTo(18.147f, 3.337f, 18.771f, 3.738f, 19.226f, 4.22f)
                curveTo(19.685f, 4.707f, 20f, 5.309f, 20f, 6f)
                verticalLineTo(15.522f)
                curveTo(19.473f, 15.446f, 18.918f, 15.611f, 18.513f, 16.016f)
                lineTo(18.5f, 16.029f)
                verticalLineTo(8.392f)
                curveTo(18.179f, 8.613f, 17.819f, 8.809f, 17.433f, 8.982f)
                curveTo(16.004f, 9.623f, 14.083f, 10f, 12f, 10f)
                curveTo(9.917f, 10f, 7.996f, 9.623f, 6.567f, 8.982f)
                curveTo(6.181f, 8.809f, 5.821f, 8.613f, 5.5f, 8.392f)
                verticalLineTo(18f)
                curveTo(5.5f, 18.207f, 5.59f, 18.459f, 5.865f, 18.75f)
                curveTo(6.144f, 19.046f, 6.582f, 19.346f, 7.18f, 19.614f)
                curveTo(7.967f, 19.966f, 8.973f, 20.239f, 10.117f, 20.384f)
                curveTo(10.204f, 20.61f, 10.338f, 20.818f, 10.513f, 20.993f)
                lineTo(11.514f, 21.993f)
                curveTo(9.619f, 21.939f, 7.884f, 21.573f, 6.567f, 20.983f)
                curveTo(5.853f, 20.663f, 5.229f, 20.262f, 4.774f, 19.78f)
                curveTo(4.315f, 19.293f, 4f, 18.691f, 4f, 18f)
                verticalLineTo(6f)
                close()
                moveTo(5.5f, 6f)
                curveTo(5.5f, 6.207f, 5.59f, 6.459f, 5.865f, 6.75f)
                curveTo(6.144f, 7.046f, 6.582f, 7.345f, 7.18f, 7.614f)
                curveTo(8.375f, 8.149f, 10.079f, 8.5f, 12f, 8.5f)
                curveTo(13.921f, 8.5f, 15.625f, 8.149f, 16.82f, 7.614f)
                curveTo(17.418f, 7.345f, 17.856f, 7.046f, 18.135f, 6.75f)
                curveTo(18.41f, 6.459f, 18.5f, 6.207f, 18.5f, 6f)
                curveTo(18.5f, 5.793f, 18.41f, 5.541f, 18.135f, 5.25f)
                curveTo(17.856f, 4.954f, 17.418f, 4.654f, 16.82f, 4.386f)
                curveTo(15.625f, 3.851f, 13.921f, 3.5f, 12f, 3.5f)
                curveTo(10.079f, 3.5f, 8.375f, 3.851f, 7.18f, 4.386f)
                curveTo(6.582f, 4.654f, 6.144f, 4.954f, 5.865f, 5.25f)
                curveTo(5.59f, 5.541f, 5.5f, 5.793f, 5.5f, 6f)
                close()
                moveTo(14.78f, 16.723f)
                curveTo(15.073f, 17.016f, 15.073f, 17.491f, 14.78f, 17.784f)
                lineTo(13.561f, 19.004f)
                horizontalLineTo(20.439f)
                lineTo(19.219f, 17.784f)
                curveTo(18.927f, 17.491f, 18.927f, 17.016f, 19.22f, 16.723f)
                curveTo(19.513f, 16.431f, 19.988f, 16.431f, 20.281f, 16.724f)
                lineTo(22.781f, 19.225f)
                curveTo(22.921f, 19.366f, 23f, 19.557f, 23f, 19.756f)
                curveTo(23f, 19.955f, 22.921f, 20.145f, 22.78f, 20.286f)
                lineTo(20.28f, 22.784f)
                curveTo(19.987f, 23.077f, 19.512f, 23.077f, 19.219f, 22.784f)
                curveTo(18.927f, 22.491f, 18.927f, 22.016f, 19.22f, 21.723f)
                lineTo(20.44f, 20.504f)
                horizontalLineTo(13.56f)
                lineTo(14.78f, 21.723f)
                curveTo(15.073f, 22.016f, 15.073f, 22.491f, 14.78f, 22.784f)
                curveTo(14.488f, 23.077f, 14.013f, 23.077f, 13.72f, 22.784f)
                lineTo(11.22f, 20.285f)
                curveTo(11.079f, 20.145f, 11f, 19.954f, 11f, 19.755f)
                curveTo(11f, 19.556f, 11.079f, 19.365f, 11.22f, 19.225f)
                lineTo(13.72f, 16.724f)
                curveTo(14.012f, 16.431f, 14.487f, 16.431f, 14.78f, 16.723f)
                close()
            }
        }.build()

        return _DatabaseSwitch!!
    }

@Suppress("ObjectPropertyName")
private var _DatabaseSwitch: ImageVector? = null

@Preview
@Composable
private fun DatabaseSwitchPreview() {
    Box(modifier = Modifier.padding(12.dp)) {
        Image(imageVector = FluentIcons.Regular.DatabaseSwitch, contentDescription = null)
    }
}
