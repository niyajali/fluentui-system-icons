package fluent.ui.system.icons.filled

import androidx.compose.foundation.Image
import androidx.compose.foundation.layout.Box
import androidx.compose.foundation.layout.padding
import androidx.compose.runtime.Composable
import androidx.compose.ui.Modifier
import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import org.jetbrains.compose.ui.tooling.preview.Preview
import androidx.compose.ui.unit.dp
import fluent.ui.system.icons.FluentIcons

public val FluentIcons.Filled.TextAlignLeftRotate90: ImageVector
    get() {
        if (_TextAlignLeftRotate90 != null) {
            return _TextAlignLeftRotate90!!
        }
        _TextAlignLeftRotate90 = ImageVector.Builder(
            name = "Filled.TextAlignLeftRotate90",
            defaultWidth = 24.dp,
            defaultHeight = 24.dp,
            viewportWidth = 24f,
            viewportHeight = 24f,
        ).apply {
            path(fill = SolidColor(Color(0xFF212121))) {
                moveTo(18f, 2f)
                curveTo(18.552f, 2f, 19f, 2.448f, 19f, 3f)
                verticalLineTo(18f)
                curveTo(19f, 18.552f, 18.552f, 19f, 18f, 19f)
                curveTo(17.448f, 19f, 17f, 18.552f, 17f, 18f)
                verticalLineTo(3f)
                curveTo(17f, 2.448f, 17.448f, 2f, 18f, 2f)
                close()
                moveTo(6f, 2f)
                curveTo(6.552f, 2f, 7f, 2.448f, 7f, 3f)
                verticalLineTo(14f)
                curveTo(7f, 14.552f, 6.552f, 15f, 6f, 15f)
                curveTo(5.448f, 15f, 5f, 14.552f, 5f, 14f)
                verticalLineTo(3f)
                curveTo(5f, 2.448f, 5.448f, 2f, 6f, 2f)
                close()
                moveTo(13f, 3f)
                curveTo(13f, 2.448f, 12.552f, 2f, 12f, 2f)
                curveTo(11.448f, 2f, 11f, 2.448f, 11f, 3f)
                verticalLineTo(21f)
                curveTo(11f, 21.552f, 11.448f, 22f, 12f, 22f)
                curveTo(12.552f, 22f, 13f, 21.552f, 13f, 21f)
                verticalLineTo(3f)
                close()
            }
        }.build()

        return _TextAlignLeftRotate90!!
    }

@Suppress("ObjectPropertyName")
private var _TextAlignLeftRotate90: ImageVector? = null

@Preview
@Composable
private fun TextAlignLeftRotate90Preview() {
    Box(modifier = Modifier.padding(12.dp)) {
        Image(imageVector = FluentIcons.Filled.TextAlignLeftRotate90, contentDescription = null)
    }
}
