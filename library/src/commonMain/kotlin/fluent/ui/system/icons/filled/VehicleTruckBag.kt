package fluent.ui.system.icons.filled

import org.jetbrains.compose.ui.tooling.preview.Preview
import androidx.compose.foundation.Image
import androidx.compose.foundation.layout.Box
import androidx.compose.foundation.layout.padding
import androidx.compose.runtime.Composable
import androidx.compose.ui.Modifier
import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import fluent.ui.system.icons.FluentUi

public val FluentUi.Filled.VehicleTruckBag: ImageVector by lazy(LazyThreadSafetyMode.NONE) {
    ImageVector.Builder(
        name = "Filled.VehicleTruckBag",
        defaultWidth = 24.dp,
        defaultHeight = 24.dp,
        viewportWidth = 24f,
        viewportHeight = 24f
    ).apply {
        path(fill = SolidColor(Color(0xFF212121))) {
            moveTo(1.75f, 4.5f)
            curveTo(1.336f, 4.5f, 1f, 4.836f, 1f, 5.25f)
            verticalLineTo(11.25f)
            curveTo(1f, 12.493f, 2.007f, 13.5f, 3.25f, 13.5f)
            horizontalLineTo(8.25f)
            curveTo(9.493f, 13.5f, 10.5f, 12.493f, 10.5f, 11.25f)
            verticalLineTo(5.25f)
            curveTo(10.5f, 4.836f, 10.164f, 4.5f, 9.75f, 4.5f)
            horizontalLineTo(9.5f)
            verticalLineTo(3.5f)
            curveTo(9.5f, 2.119f, 8.381f, 1f, 7f, 1f)
            curveTo(6.545f, 1f, 6.118f, 1.122f, 5.75f, 1.335f)
            curveTo(5.382f, 1.122f, 4.955f, 1f, 4.5f, 1f)
            curveTo(3.119f, 1f, 2f, 2.119f, 2f, 3.5f)
            verticalLineTo(4.5f)
            horizontalLineTo(1.75f)
            close()
            moveTo(7f, 4.5f)
            verticalLineTo(3.5f)
            curveTo(7f, 3.152f, 6.929f, 2.821f, 6.801f, 2.52f)
            curveTo(6.865f, 2.507f, 6.932f, 2.5f, 7f, 2.5f)
            curveTo(7.552f, 2.5f, 8f, 2.948f, 8f, 3.5f)
            verticalLineTo(4.5f)
            horizontalLineTo(7f)
            close()
            moveTo(5.5f, 3.5f)
            verticalLineTo(4.5f)
            horizontalLineTo(3.5f)
            verticalLineTo(3.5f)
            curveTo(3.5f, 2.948f, 3.948f, 2.5f, 4.5f, 2.5f)
            curveTo(5.052f, 2.5f, 5.5f, 2.948f, 5.5f, 3.5f)
            close()
            moveTo(2f, 17.25f)
            verticalLineTo(14.251f)
            curveTo(2.385f, 14.411f, 2.807f, 14.5f, 3.25f, 14.5f)
            horizontalLineTo(8.25f)
            curveTo(10.045f, 14.5f, 11.5f, 13.045f, 11.5f, 11.25f)
            verticalLineTo(5.25f)
            curveTo(11.5f, 4.552f, 11.091f, 3.949f, 10.5f, 3.668f)
            verticalLineTo(3.5f)
            curveTo(10.5f, 3.33f, 10.488f, 3.163f, 10.465f, 3f)
            horizontalLineTo(14.25f)
            curveTo(15.493f, 3f, 16.5f, 4.007f, 16.5f, 5.25f)
            verticalLineTo(6f)
            horizontalLineTo(18.051f)
            curveTo(18.92f, 6f, 19.712f, 6.501f, 20.084f, 7.287f)
            lineTo(21.783f, 10.874f)
            curveTo(21.926f, 11.175f, 22f, 11.504f, 22f, 11.837f)
            verticalLineTo(17.25f)
            curveTo(22f, 18.493f, 20.993f, 19.5f, 19.75f, 19.5f)
            horizontalLineTo(17.958f)
            curveTo(17.721f, 20.919f, 16.486f, 22f, 15f, 22f)
            curveTo(13.514f, 22f, 12.28f, 20.919f, 12.042f, 19.5f)
            horizontalLineTo(9.959f)
            curveTo(9.72f, 20.919f, 8.486f, 22f, 7f, 22f)
            curveTo(5.51f, 22f, 4.274f, 20.914f, 4.04f, 19.49f)
            curveTo(2.896f, 19.384f, 2f, 18.422f, 2f, 17.25f)
            close()
            moveTo(20.5f, 15.5f)
            horizontalLineTo(19.25f)
            curveTo(18.836f, 15.5f, 18.5f, 15.836f, 18.5f, 16.25f)
            curveTo(18.5f, 16.664f, 18.836f, 17f, 19.25f, 17f)
            horizontalLineTo(20.5f)
            verticalLineTo(15.5f)
            close()
            moveTo(20.183f, 11f)
            lineTo(18.729f, 7.929f)
            curveTo(18.604f, 7.667f, 18.341f, 7.5f, 18.051f, 7.5f)
            horizontalLineTo(16.5f)
            verticalLineTo(11f)
            lineTo(20.183f, 11f)
            close()
            moveTo(7f, 20.5f)
            curveTo(7.828f, 20.5f, 8.5f, 19.828f, 8.5f, 19f)
            curveTo(8.5f, 18.172f, 7.828f, 17.5f, 7f, 17.5f)
            curveTo(6.172f, 17.5f, 5.5f, 18.172f, 5.5f, 19f)
            curveTo(5.5f, 19.828f, 6.172f, 20.5f, 7f, 20.5f)
            close()
            moveTo(16.5f, 19f)
            curveTo(16.5f, 18.172f, 15.828f, 17.5f, 15f, 17.5f)
            curveTo(14.172f, 17.5f, 13.5f, 18.172f, 13.5f, 19f)
            curveTo(13.5f, 19.828f, 14.172f, 20.5f, 15f, 20.5f)
            curveTo(15.828f, 20.5f, 16.5f, 19.828f, 16.5f, 19f)
            close()
            moveTo(3.5f, 3.5f)
            curveTo(3.5f, 3.358f, 3.53f, 3.223f, 3.583f, 3.101f)
            close()
        }
    }.build()
}

@Preview
@Composable
private fun VehicleTruckBagPreview() {
    Box(modifier = Modifier.padding(12.dp)) {
        Image(imageVector = FluentUi.Filled.VehicleTruckBag, contentDescription = null)
    }
}
